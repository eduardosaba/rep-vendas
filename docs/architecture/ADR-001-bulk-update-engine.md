...
* **Status:** Aceito / Em Homologação (Modo 1)
* **Data:** 2026-07-27
...
2. **Defesa em Profundidade nas RPCs:** Funções PostgreSQL com `SECURITY DEFINER`, `SET search_path = public, pg_temp`, checagem de permissão por `profiles.role` via `IF NOT EXISTS` e bloqueio de linha por exclusão mútua (`FOR UPDATE`).
...
- **Reaproveitamento de Engenharia:** Cerca de 80% da infraestrutura de backend (streaming, hashing, idempotência, locks, auditoria e rollback) será 100% reutilizada nos próximos módulos.
...
- **Marco 2 (Concluído / Em Homologação):** Modo 1 — Linha da Fábrica (`is_active`).
