...
export function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    // Mostra apenas se não estiver em modo standalone (já instalado)
    ...
    <div className="fixed bottom-0 left-0 right-0 z-[60] bg-white border-t border-gray-200 p-4 shadow-2xl md:hidden animate-in slide-in-from-bottom">
      <div className="flex items-center justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500">Acesse o catálogo mais rápido.</p>
        </div>
        
        {isIOS ? (
           <div className="text-xs text-gray-600 text-right">
             Toque em <strong>Compartilhar</strong> <br/> e <strong>Adicionar à Tela de Início</strong>
           </div>
        ) : (
           <button 
        ...
        <button onClick={() => setIsVisible(false)} className="p-1 text-gray-400 hover:text-gray-600">
           <X size={20} />
        </button>
      </div>
    </div>
