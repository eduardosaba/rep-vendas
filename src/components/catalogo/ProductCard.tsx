            ...
            <div className="flex items-center justify-between gap-2 w-full">
              {isPricesVisible ? (
                <div className="flex flex-col">
                  <PriceDisplay value={currentPrice} priceOnRequest={product.price_on_request} isPricesVisible={isPricesVisible} className="text-lg font-black text-gray-900 leading-tight" />
                  {hasValidOriginalPrice && !product.price_on_request && (<div className="mt-1 text-xs text-gray-500 line-through"><PriceDisplay value={product.original_price!} isPricesVisible={isPricesVisible} className="text-sm font-medium" size="small" /></div>)}
                  <div className="mt-1">{(() => { const isUsingSalePrice = currentPrice === salePrice && salePrice > 0; const showInstallmentsSetting = (storeSettings as any).show_installments === true; const maxInst = Number((storeSettings as any).max_installments || 0); if (!isPricesVisible || product.price_on_request) return null; if (isUsingSalePrice) { if (!showInstallmentsSetting) return null; return getInstallmentText(currentPrice, (storeSettings as any).max_installments || 1, isPricesVisible, product.price_on_request); } if (showCost && showInstallmentsSetting && maxInst > 1) { return getInstallmentText(currentPrice, maxInst, isPricesVisible, product.price_on_request); } return null; })()} {getCashDiscountText(currentPrice, discountPercent, isPricesVisible, product.price_on_request)}</div>
                </div>
              ) : (
