...
export default function ProductDetailPage() {
  ...
  // Compatibilidade de rota: aceita /catalogo/[slug] e /catalogo/[slug]/[repSlug].
  const slugParam =
    (params?.slug as string | string[] | undefined) ||
    ((params?.companySlug && params?.repSlug)
      ? [String(params.companySlug), String(params.repSlug)]
      : undefined);
  const slugParts = useMemo(
    () => (Array.isArray(slugParam) ? slugParam.filter(Boolean) : slugParam ? [slugParam] : []),
    [slugParam]
  );
  const slugKey = slugParts.join('/');
  const defaultCatalogPath =
    slugParts.length >= 2
      ? `/catalogo/${slugParts[0]}/${slugParts[1]}`
      : slugParts.length === 1
        ? `/catalogo/${slugParts[0]}`
        : '/catalogo';
  const productIdParam = params?.productId as string | string[] | undefined;
  const productId = Array.isArray(productIdParam) ? productIdParam[0] : (productIdParam || '');
  const router = useRouter();
  const [catalogPathPrefix, setCatalogPathPrefix] = useState(defaultCatalogPath);
  ...
  useEffect(() => {
    setCatalogPathPrefix(defaultCatalogPath);
  }, [defaultCatalogPath]);

  useEffect(() => {
    if (slugParts.length > 0 && productId) {
  ...
  const loadStoreAndProduct = async () => {
    try {
      const context = await resolveContext(slugParts, supabase as any);
      if (!context) throw new Error('Contexto do catálogo não encontrado');

      setCatalogPathPrefix(context.pathPrefix);
      const settingsSlug = context.catalogSlug;

      // Buscar configurações da loja usando catalog_slug
      const { data: store, error: storeError } = await supabase
        .from('settings')
        .select('*')
        .eq('catalog_slug', settingsSlug)
        .maybeSingle();
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-100">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push(catalogPathPrefix)}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between py-4">
            ...
            <div className="flex items-center space-x-6">
              ...
              <button
                onClick={() => router.push(`${catalogPathPrefix}/checkout`)}
                className="flex flex-col items-center text-gray-600 hover:text-gray-900"
                style={{ color: settings?.icon_color || '#4B5563' }}
              >
