...
export default async function MaintenancePage({ params }: Props) {
  ...
  const { data: catalog } = await supabase
    ...
    .eq('catalog_slug', context.catalogSlug)
    .maybeSingle();

  const fallbackName =
    context.company?.name ||
    context.settings?.name ||
    context.catalog?.store_name ||
    context.representative?.display_name ||
  ...
  const effectiveCatalog =
    ...
    ({
      ...
      logo_url: context.company?.logo_url || context.settings?.logo_url || context.catalog?.logo_url || null,
      primary_color: context.company?.primary_color || context.settings?.primary_color || '#b9722e',
      secondary_color: context.company?.secondary_color || context.settings?.secondary_color || '#0d1b2c',
    } as any);

  const primaryColor = effectiveCatalog.primary_color || '#b9722e';
  const secondaryColor = effectiveCatalog.secondary_color || '#0d1b2c';
