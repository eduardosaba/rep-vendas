      ...
      >
        {isLoading ? (
          <span className="inline-flex items-center">
            ...
            <span className="opacity-90">{loadingText || 'Carregando...'}</span>
          </span>
        ) : (
          <span className="inline-flex items-center">
            {leftIcon && <span className="mr-2 -ml-1 opacity-90">{leftIcon}</span>}
            {children}
            {rightIcon && <span className="ml-2 -mr-1 opacity-90">{rightIcon}</span>}
          </span>
        )}
      </button>
    );
