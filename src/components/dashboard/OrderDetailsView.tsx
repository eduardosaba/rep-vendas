...
import { ChevronDown, Package, X, ZoomIn } from 'lucide-react';
...
export function OrderDetailsView({ order }: { order: any }) {
  ...
  >({});
  const [brandLogoFailed, setBrandLogoFailed] = React.useState<
  ...
  const getProductGalleryImage = (item: any) => {
    const pics =
      item?.products?.product_images || item?.products?.gallery_images || null;
    ...
    return (
      ...
      primary.optimized_variants?.find?.((v: any) => Number(v?.size) === 480)
        ?.url ||
      primary.optimized_variants?.[0]?.url ||
      null
    );
  };

  const getBestItemImage = (item: any, width: number = 480) => {
    const variants =
      item?.optimized_variants ||
      item?.image_variants ||
    ...
    if (Array.isArray(variants) && variants.length > 0) {
      const exact = variants.find((v: any) => Number(v?.size) === width);
      const large = variants.find((v: any) => Number(v?.size) >= width);
      const chosen = exact || large || variants[0];
      const variantUrl =
        chosen?.url ||
        (chosen?.path ? buildSupabaseImageUrl(chosen.path, { width }) : null);
    ...
    const candidate =
      ...
      getProductGalleryImage(item);

    if (typeof candidate === 'string' && candidate.trim()) {
      return (
        buildSupabaseImageUrl(candidate, { width }) || safeImageUrl(candidate)
      );
    }

    return safeImageUrl(null);
  };
  ...
  const openLightbox = (src: string, product: any) =>
    setLightbox({ src, product });
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-gray-100 dark:divide-slate-800">
          {Object.keys(groups).map((brand) => {
            ...
            if (!brandLogo) {
              for (const it of items) {
                const pics =
                  ...
                  it?.products?.gallery_images ||
                  null;
                if (Array.isArray(pics) && pics.length > 0) {
                  const primary =
                    pics.find((p: any) => p.is_primary) || pics[0];
                  if (primary) {
                    const cand =
                      primary.url ||
                      primary.path ||
                    ...
                    if (cand) {
                      const resolved =
                        buildSupabaseImageUrl(cand) ||
                        (typeof cand === 'string' &&
                          (cand.startsWith('http') ? cand : null));
                      if (resolved) {
                        brandLogo = resolved;
                        break;
                      }
            ...
            const groupTotalUnits = items.reduce(
              (a: number, i: any) => a + (i.quantity || 0),
              0
            );
            const groupTotalValue = items.reduce(
              (a: number, i: any) =>
                a + (i.quantity || 0) * (i.unit_price || i.price || 0),
              0
            );
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-sm text-gray-500">
                          {groupTotalUnits} un. — {fmt.format(groupTotalValue)}
                        </div>
                        <button
                          aria-expanded={!collapsed[brand]}
                          onClick={() =>
                            setCollapsed((s) => ({ ...s, [brand]: !s[brand] }))
                          }
                          className="p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-800"
                        >
                          <ChevronDown
                            className={`transition-transform ${collapsed[brand] ? '-rotate-90' : 'rotate-0'}`}
                            size={16}
                          />
                        </button>
                      </div>
