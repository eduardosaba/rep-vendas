...
export default async function OrdersPage() {
    ...
    if (!companyOrders.success) {
      return (
        <div className="p-6 text-red-600">
          Erro ao carregar pedidos centralizados: {companyOrders.error}
        </div>
      );
    }
  ...
  if (isRepresentative && companyId) {
    ...
    const mySales = mapOrders(mySalesRows || []);
    const distributorOrders = mapOrders(distributorRows || []);
    ...
    return (
      <div className="p-4 md:p-6 space-y-6 pb-24 animate-in fade-in duration-500">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Gestão por carteira: vendas diretas e orçamentos da distribuidora.
            </p>
          </div>
        ...
        <RepOrdersTabs mySales={mySales} distributorOrders={distributorOrders} currentUserId={finalUser.id} />
      </div>
    );
  }
