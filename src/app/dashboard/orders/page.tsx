...
import { ArrowLeft, Package, Truck, CheckCircle, Clock, AlertCircle, Search, Filter, Edit, Save, X } from 'lucide-react'
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600">{filteredOrders.length} pedidos encontrados</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div key={order.id} className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-900">{order.clients?.name || 'Cliente não informado'}</p>
                  {order.clients?.email && (
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900 capitalize">
                    {order.order_type === 'catalog' ? 'Catálogo' : order.order_type === 'quick_brand' ? 'Pedido Rápido' : order.order_type}
                  </p>
                </div>
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Pagamento</h4>
                  <p className="text-sm text-gray-900 capitalize">
                    {order.payment_method === 'boleto' ? 'Boleto' : order.payment_method === 'pix' ? 'PIX' : order.payment_method === 'cartao' ? 'Cartão' : order.payment_method || 'Não informado'}
                  </p>
                </div>
              </div>
              ...
              {order.delivery_address && (
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-1">Endereço de Entrega</h4>
                ...
                <div className="space-y-2">
                  {order.order_items?.map((item, index) => (
                    <div key={index} className="flex justify-between text-sm">
                      <span>{item.products?.name} {item.products?.brand ? `(${item.products.brand})` : ''}</span>
                      <span>{item.quantity}x R$ {item.unit_price?.toFixed(2)} = R$ {item.total_price?.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {editingOrder === order.id ? (
                <div className="border-t pt-4 mt-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Editar Pedido</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditData(prev => ({ ...prev, tracking_code: e.target.value }))}
                        className="w-full border border-gray-300 rounded px-3 py-2"
                        placeholder="Ex: BR123456789BR"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Data Estimada de Entrega</label>
                      <input
                        type="date"
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      {order.estimated_delivery && (
                        <p className="text-sm text-gray-600">
                          <strong>Entrega Estimada:</strong> {new Date(order.estimated_delivery).toLocaleDateString('pt-BR')}
                        </p>
                      )}
                      {order.notes && (
                        <p className="text-sm text-gray-600">
        ...
        {filteredOrders.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {orders.length === 0 ? 'Você ainda não tem pedidos.' : 'Tente ajustar os filtros de busca.'}
            </p>
          </div>
        )}
      </div>
