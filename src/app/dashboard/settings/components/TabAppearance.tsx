        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-gray-200 shadow-sm space-y-6">
            ...
            {THEME_PRESETS.map((theme: any, idx: number) => (
                ...
                onClick={() => {
                  ...
                  try {
                    setFormData((prev: any) => ({
                      ...
                      ...(theme.footer_background_color ? { footer_background_color: theme.footer_background_color } : {}),
                      ...(theme.footer_text_color ? { footer_text_color: theme.footer_text_color } : {}),
                      ...(theme.footerBackground ? { footer_background_color: theme.footerBackground } : {}),
                      ...(theme.footerTextColor ? { footer_text_color: theme.footerTextColor } : {}),
                    }));
                  } catch (e) {
                  ...
                  toast.success(`Tema ${theme.name} selecionado! Clique em salvar.`);
                }}
              ...
              >
                <div className="flex h-10 w-full rounded-lg overflow-hidden border border-slate-200">
                  ...
                  <div className="w-1/4 h-full" style={{ backgroundColor: theme.header }} />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-4">
              <label className="text-xs font-black uppercase text-slate-500">Logo da Marca</label>
              <div className="h-48 w-full border-2 border-dashed rounded-[2rem] flex items-center justify-center bg-slate-50 relative group">
                {logoPreview ? (
                  <img src={logoPreview} className="max-h-full p-6 object-contain" />
                ) : (
                ...
                <label className="absolute inset-0 cursor-pointer flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-[2rem]">
                  <span className="text-white font-black text-[10px] uppercase tracking-widest bg-primary px-4 py-2 rounded-xl">Alterar Logo</span>
                  <input
                    type="file"
                    className="hidden"
                    accept="image/*"
                    onChange={async (e: any) => {
                      ...
                      try {
                        ...
                        try {
                          setFormData((p: any) => ({ ...p, logo_url: data.publicUrl }));
                        } catch (e) {
                          // ignore if setFormData not provided
                        }
                        ...
                        toast.error('Falha no upload da logo: ' + (err?.message || String(err)));
                      }
                    }}
                  />
                </label>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <select
                  ...
                  onChange={async (e: any) => {
                    ...
                    const preset = _sf.find((p: any) => p.name === selectedName) || null;
                    setFormData((p: any) => ({ ...p, font_family: selectedName, font_url: preset?.import ?? null }));
                    try {
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    onChange={(e: any) => setFormData((p: any) => ({ ...p, font_url: e.target.value || null }))}
          ...
          <div className="mt-6 bg-white dark:bg-slate-900 p-4 rounded-[1rem] border border-gray-200 shadow-sm">
            ...
            <div className="flex gap-2">
              {['fit', 'fill', 'stretch'].map((mode) => (
                ...
                >
                  {mode === 'fit' ? 'Ajustar (Contain)' : mode === 'fill' ? 'Preencher (Cover)' : 'Esticar'}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4 pt-6 border-t">
            <div className="flex items-center justify-between">
              <label className="text-xs font-black uppercase text-slate-500">Banners Mobile (768x400)</label>
              <label className="px-4 py-2 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase cursor-pointer hover:bg-primary transition-colors">Adicionar <input type="file" className="hidden" accept="image/*" multiple onChange={async (e: any) => {
                const files = Array.from(e.target.files || []);
                if (!files.length) return;
                try {
