...
export default async function IndividualSuccessPage({
  ...
  const context = await resolveContext([resolvedParams.slug], supabase as any);

  if (!context || context.type !== 'individual' || !context.representative?.id) {
    return notFound();
  }
  ...
  const basePath = `/catalogo/${resolvedParams.slug}`;
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-10 sm:px-6">
      <div className="mx-auto max-w-xl rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-center text-sm text-slate-500">
          Atendimento finalizado com {context.representative.full_name || context.representative.slug}.
        </p>
        ...
        {receiptOrder ? (
          <div className="mt-6">
            <OrderReceipt
              order={receiptOrder}
              company={{
                name: context.representative.full_name || context.representative.slug,
              }}
              representative={{
