...
export const metadata = {
  title: 'Tabela de Produtos (Torre) | Rep-Vendas',
  description: 'Edição global de produtos (Torre de Controle).',
};

export const dynamic = 'force-dynamic';

export default async function AdminBulkEditPage({ searchParams }: any) {
  const supabase = await createClient();

  // NOTE: this page is intended for use by the master/admin. It's the
  // "control tower" view — we intentionally do not scope by the current
  // session user so operators can edit global source products.
  ...
  if (brandFilter) query.eq('brand', brandFilter);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-1rem)] bg-gray-50 dark:bg-slate-950 p-4 md:p-6 overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Edição global de produtos — alterações afetam a fonte do catálogo.
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col relative">
        {products.length > 0 ? (
          // BulkEditClient upserts with `user_id` provided in payload. In the
          // admin view we provide the product rows as-is so that the operator
          // can edit the source records directly.
          <BulkEditClient initialProducts={products as any} />
        ) : (
