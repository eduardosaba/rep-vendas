import { NextResponse } from 'next/server';
import { createClient as createServiceClient } from '@supabase/supabase-js';

export async function GET(req: Request) {
  try {
    const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL;
    const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
      return NextResponse.json({ error: 'Server not configured' }, { status: 500 });
    }

    const supabase = createServiceClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

    const url = new URL(req.url);
    const sourceUser = url.searchParams.get('sourceUser');

    let query: any = supabase.from('products').select('brand');
    query = query.neq('brand', null);
    if (sourceUser) query = query.eq('user_id', sourceUser);

    const { data, error } = await query;
    if (error) throw error;

    const counts = new Map<string, number>();
    (data || []).forEach((r: any) => {
      if (r.brand) counts.set(r.brand, (counts.get(r.brand) || 0) + 1);
    });

    const list = Array.from(counts.entries()).map(([name, count]) => ({ name, count }));
    list.sort((a, b) => a.name.localeCompare(b.name));

    return NextResponse.json({ brands: list });
  } catch (err: any) {
    console.error('brand-counts error', err);
    return NextResponse.json({ error: err?.message || String(err) }, { status: 500 });
  }
}
