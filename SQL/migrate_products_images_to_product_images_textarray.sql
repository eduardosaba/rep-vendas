-- Migração: expande products.images (text[]) e insere em product_images marcando como 'pending'
BEGIN;
...
INSERT INTO public.product_images (product_id, url, is_primary, position, sync_status, created_at)
SELECT product_id, url, is_primary, position, 'pending', now()
FROM to_insert
WHERE NOT EXISTS (
