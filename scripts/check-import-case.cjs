...
function main() {
  ...
  for (const file of sourceFiles) {
    ...
    const re = /(?:from\s+|import\s+|require\()(['"])(\.\.\/|\.\/)([^'\"]+)\1/g;
    while ((match = re.exec(content)) !== null) {
      ...
      if (found) {
        const actualRel = path
          ...
          .replace(/\\/g, '/');
        const importPathUsed = spec.replace(/\\/g, '/');
        const actualRelNoExt = actualRel.replace(
          /(\/index)?(\.tsx?|\.jsx?|\.mjs|\.cjs)$/,
          ''
        );
        const usedNormalized = importPathUsed
          .replace(/^(?:\.\/)+/, '')
          .replace(/(\.tsx?|\.jsx?|\.mjs|\.cjs)$/, '');
        const actualNormalized = actualRelNoExt.replace(/^(?:\.\/)+/, '');
        if (usedNormalized !== actualNormalized) {
          problems.push({
            file,
            import: spec,
            resolved: found,
