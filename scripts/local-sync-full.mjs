...
function splitUrls(input) {
  ...
  return String(input).split(/[;,]/).map(u => u.trim()).filter(u => u.startsWith('http'));
}
...
async function processImage(url, storageBase, agent) {
  ...
  return Promise.all(RESPONSIVE_SIZES.map(async (size) => {
...
async function syncFullCatalog() {
  ...
  let query = supabase.from('products').select('id, reference_code, image_url, external_image_url, images, brand')
    .or('sync_status.eq.pending,sync_status.eq.failed');

  if (brandFilter) query = query.ilike('brand', `%${brandFilter}%`);

  const { data, error } = await query.limit(1000);
  if (error || !data) { console.error('❌ Erro Supabase:', error?.message); return; }
  ...
  const tasks = data.map((product) => 
    limit(async () => {
      ...
      try {
        const urls = [...new Set([...splitUrls(product.image_url), ...splitUrls(product.external_image_url), ...splitUrls(product.images)])];

        if (urls.length > 0) {
          // Processa Capa
          const mainVariants = await processImage(urls[0], `${storagePath}-main`, agent);
          
          // Processa Galeria em lote interno também
          const gallery = [];
          const galleryUrls = urls.slice(1);
          await Promise.all(galleryUrls.map(async (gUrl, idx) => {
            try {
              const v = await processImage(gUrl, `${storagePath}-${idx+2}`, agent);
              gallery.push({ 
          ...
          await supabase.from('products').update({
            ...
            image_url: null, external_image_url: null, images: null,
            image_optimized: true,
            updated_at: new Date().toISOString()
          }).eq('id', product.id);
        } else {
          await supabase.from('products').update({ sync_status: 'synced', sync_error: 'Sem URLs' }).eq('id', product.id);
        }
        
        processed++;
        // Log minimalista para não travar o buffer do terminal
        process.stdout.write(`\r✅ Processados: ${processed}/${data.length} | Atual: ${ref}          `);

      } catch (err) {
        processed++;
        console.error(`\n❌ Erro em ${ref}: ${err.message}`);
        await supabase.from('products').update({ sync_status: 'failed', sync_error: err.message }).eq('id', product.id);
      }
    })
  );
  ...
  notifier.notify({ title: 'RepVendas Turbo', message: `Concluído em ${duration}s.` });
}
